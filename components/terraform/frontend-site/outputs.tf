...
output "delegated_hosted_zone_id" {
  ...
  value       = var.dns.hosted_zone.create ? module.delegated_zone[0].zone_id : null
}

output "delegated_hosted_zone_name" {
  description = "Name of the delegated environment hosted zone, when created."
  value       = var.dns.hosted_zone.create ? module.delegated_zone[0].zone_name : null
}
...
output "delegated_hosted_zone_name_servers" {
  ...
  value       = var.dns.hosted_zone.create ? module.delegated_zone[0].name_servers : []
}
...
output "basic_auth_ssm_parameter_name" {
  ...
  value       = var.access.basic_auth.create_edge_lambda ? module.basic_auth_edge[0].ssm_parameter_name : var.access.basic_auth.ssm_parameter_name
}
